...
MapperProxy 代理mapper接口，对mapper接口的方法进行拦截(只拦截非默认方法)  
...
//从mapperRegistry中获取mapper，mapperRegistry是在读取配置文件的时候初始化
mapperRegistry.getMapper(type, sqlSession);

//在mapperRegistry中获取代理对象
//knownMappers的key是对应mapper接口的class对象，value是代理工厂
...
//methodCache key是被拦截的方法对象，value是MapperMethod，该对象会包装SqlCommand、MethodSignature
// execute主要是根据标签类型执行对应的sqlSession的方法，查询的话比较复杂，需要再判断返回值的类型再进行对应方法的调用
```
 1).  MapperMethod中的SqlCommand: 主要是得到标签的类型和标签对应的路径  
 
 2).  MapperMethod中的MethodSignature: 封装方法的返回值和入参类型
